...
#
#   NAME=$PRE-4-$DATAT
#   docker run -d $DOCKER_ARGS --name $NAME $PRE ./v34/train.py ex/$NAME data/conll16st-$DATAT data/conll16st-$DATAV --clean --config="{\"focus_dim\":\"4\", $CONFIG}" && echo -ne "\ek${NAME:10}\e\\" && sleep 5 && less +F $PREDIR/ex/$NAME/console.log
#     docker logs -f $NAME
#     docker rm -f $NAME
